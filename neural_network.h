...
float xavier_init(uint32_t nin, uint32_t nout){
    float lim = sqrtf((float)6/(nin+nout));
    return RandomFloat(-lim, +lim);
}
...
struct NETWORK* init_network(uint32_t* ncnt, uint32_t lcnt, uint32_t input_cnt){
    ...
    for(int j = 0; j < ncnt[0]; j++) N->L[0].biases[j] = xavier_init(0.5f, 2.0f);
    
    N->L[0].Z = calloc(ncnt[0], sizeof(float));

    N->L[0].weights = calloc(ncnt[0]*input_cnt, sizeof(float));
    for(int i = 0; i < ncnt[0]*input_cnt; i++) N->L[0].weights[i] = xavier_init(input_cnt, ncnt[0]);
    ...
    for(int i = 1; i < lcnt; i++){
        ...
        for(int j = 0; j < ncnt[i]; j++) N->L[i].biases[j] = xavier_init(0.5f, 2.0f);

        N->L[i].Z = calloc(ncnt[i], sizeof(float));

        N->L[i].weights = calloc(ncnt[i]*ncnt[i-1], sizeof(float));
        for(int j = 0; j < ncnt[i]*ncnt[i-1]; j++) N->L[i].weights[j] = xavier_init(ncnt[i-1], ncnt[i]);
    }
